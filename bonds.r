...
portfolio <- function(bonds) {
  tryCatch(
    expr = {
      ...
      if (p != 1) {
        cat("Sum of shares allocated",p)
        stop(" Sum of allocation is not 1")
      }
      ...
      cat(
        ...
        paste(paste(100 * as.numeric(
          purrr::map(name_bonds, 'weight')
        ), '%') , names(name_bonds)) ,
        "Portfolio value",
        paste(round(p_price, 2), '$'),
        "Portfolio yield",
        paste(round(100 * p_ytm, 2), '%'),
        "Portfolio  sensitvity to 25 basis
        point change in yield",
        paste(round(100 * sens, 2), '%'),
        "Portfolio
        duration",
        paste(round(p_dur, 2), "years"),
        fill = 3
      )
